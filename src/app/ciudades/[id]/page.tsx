...
export default async function CityPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main className="city-page" id="main-content">
    ...
    <section className="city-hero"><div><p className="eyebrow">{city.province} · {city.region}</p><h1>{city.name}</h1><p>{city.summary}</p></div><aside><span>Rango mensual estimado</span><b>{city.costRange}</b><small>Snapshot al {formattedDate}</small></aside></section>
    <section className="city-metrics"><header><h2>Señales comparables</h2><p>Índices 0–100 dentro de estas 24 ciudades. No son estadísticas absolutas.</p></header>{FACTORS.map((factor) => <article key={factor}><div><h3>{factorLabels[factor]}</h3><strong>{city.metrics[factor]}</strong></div><meter min="0" max="100" value={city.metrics[factor]}>{city.metrics[factor]}</meter><small>{city.sourceIds[factor]?.map((sourceId) => dataSources.find((source) => source.id === sourceId)?.name).filter(Boolean).join(" · ")}</small></article>)}</section>
