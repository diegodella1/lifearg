...
export default async function CityPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main className="city-page" id="main-content">
    ...
    <section className="city-hero"><div><p className="eyebrow">{city.province} · {city.region}</p><h1>{city.name}</h1><p>{city.summary}</p></div><aside className="city-dossier"><CityLocator city={city}/><div className="city-cost"><span>Rango mensual estimado</span><b>{city.costRange}</b><small>Snapshot al {formattedDate}</small></div></aside></section>
