...
export function TaxEstimator({ results }: { results: MatchResult[] }) {
  ...
  return <section className="tax-estimator" aria-labelledby="tax-title">
    ...
    <div className="tax-layout">
      <div className="tax-controls">
        <label><span>Ciudad evaluada</span><select value={city.id} onChange={(event) => setCityId(event.target.value)}>{results.map((result) => <option key={result.city.id} value={result.city.id}>{result.city.name}, {result.city.province}</option>)}</select></label>
        <label><span>Situación</span><select value={status} onChange={(event) => setStatus(event.target.value as TaxStatus)}><option value="employee">Empleado/a en relación de dependencia</option><option value="monotributo_services">Monotributista de servicios</option><option value="no_work_income">Sin ingreso laboral</option></select></label>
        {status !== "no_work_income" && <label><span>{status === "employee" ? "Sueldo bruto mensual" : "Facturación mensual promedio"}</span><input inputMode="numeric" min="0" max="1000000000" placeholder="Ejemplo: 1500000" type="number" value={monthlyGross ?? ""} onChange={(event) => setMonthlyGross(event.target.value ? Number(event.target.value) : undefined)}/></label>}
        <div className="tax-jurisdiction"><CircleAlert size={17}/><p><b>{city.province}:</b> Ingresos Brutos y tasas locales no están incluidos. Dependen de actividad, régimen, exenciones y domicilio fiscal.</p></div>
      </div>
      <div className="tax-result" aria-live="polite">
        {!estimate && status !== "no_work_income" ? <div className="tax-empty"><Calculator size={28}/><p>Ingresá un monto para ver el orden de magnitud mensual.</p></div> : <TaxResult status={status} estimate={estimate ?? estimatePersonalTaxes({ status, monthlyGross: 0 })}/>} 
      </div>
    </div>
...
function TaxResult({ status, estimate }: { status: TaxStatus; estimate: ReturnType<typeof estimatePersonalTaxes> }) {
  if (estimate.monthlyTotal === null) return <div className="tax-empty tax-empty--warn"><CircleAlert size={28}/><p><b>Fuera del rango estimable.</b>{estimate.notes[0]}</p></div>;
